...
export default function LoginPage() {
  const [email, setEmail] = useState("admin@najmo.dz");
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        setError("Email ou mot de passe incorrect. Essayez : admin@najmo.dz / admin123");
      } else if (result?.ok) {
        router.push("/");
        router.refresh();
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#080C14] text-white p-4">
      <div className="max-w-md w-full space-y-8 bg-[#141E2E] p-10 rounded-2xl shadow-2xl border border-[rgba(255,255,255,0.08)]">
        ...
        <div className="mt-4 p-4 bg-[#080C14] border border-[rgba(255,255,255,0.06)] rounded-xl text-xs text-[#4A5878]">
          ...
          <p>Email : <span className="text-[#3B82F6] font-mono">admin@najmo.dz</span></p>
          <p>Mot de passe : <span className="text-[#3B82F6] font-mono">admin123</span></p>
        </div>
      </div>
    </div>
  );
